<x-layouts::app :title="__('Edit Mitra')">
    <div class="max-w-7xl mx-auto pb-20">
        ...
        <div class="mb-8 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-zinc-900 dark:text-white">Edit Mitra</h1>
                <p class="text-zinc-500 text-sm mt-1">{{ $mitra->company_name }}</p>
            </div>
        </div>

        {{-- Form --}}
        <form action="{{ route('mitras.update', $mitra->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-700 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="company_name" value="{{ old('company_name', $mitra->company_name) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="contact_person" value="{{ old('contact_person', $mitra->contact_person) }}"
                            class="w-full rounded-lg border border-zinc-300 bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-800 dark:text-white py-2.5 px-4 focus:ring-2 focus:ring-zinc-900 focus:border-transparent transition"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $mitra->phone) }}"
                                class="w-full rounded-lg border border-zinc-300 bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-800 dark:text-white py-2.5 px-4 focus:ring-2 focus:ring-zinc-900 focus:border-transparent transition"
                    ...
                    <div>
                        ...
                        <textarea name="address" rows="3"
                            class="w-full rounded-lg border border-zinc-300 bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-800 dark:text-white py-2.5 px-4 focus:ring-2 focus:ring-zinc-900 focus:border-transparent transition"
                            placeholder="Alamat lengkap perusahaan">{{ old('address', $mitra->address) }}</textarea>
                        @error('address')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
